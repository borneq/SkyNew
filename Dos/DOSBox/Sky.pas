...
TYPE s15=STRING[15];
     funrr=FUNCTION(x:double):double;
CONST
...
VAR jd,t1,t,dl,db,dr,dlr,lr,t2,ep,dbr,epr,x1,x2,x3,x4,x5,x6,al,de,gr:double;
    jr:ARRAY[1..33] OF double; pm,kierunek,p4,p5,p6,az,wys,ttt,jdata,Refr:double;
    kk,jj,Opcje,Obiekt,Paralaksa,rpm,mpm,dpm,gd,md,sd,dt,gobi,mibi,sebi,
    rbi,mbi,dbi:INTEGER;
    ch,ch1,cpres:CHAR;
    jDNIA,xob,yob,zob, sze, dlu :double;
    ...
    Coor,LoCoor: ARRAY [0..10,'w'..'z'] OF double;
    ask:BOOLEAN;
    jd1,jd2,jd3,w1,w2,w3:double;
...
CONST dtyg:ARRAY[1..7]OF S3=('Pon','Wt ','Sr ','Czw','Pia','Sob','N');
      Pi:double=3.141592653589793238;s:double=0.017453292519943295769;
...
PROCEDURE minimum(a,b,d:double; VAR x:double; f:funrr);  { zloty podzial }
          { a,b - przedzial, d - dokladnosc, x - wartosc x, f - funkcja }
VAR gkl,gkp,gk1l,gk1p,
    akl,akp,ak1l,ak1p,
    qakl,qakp,qak1l,qak1p:double;
...
PROCEDURE RokMD(jd: double; VAR r,m,d:INTEGER; VAR gr:double);
VAR w,x,u,z,y,a,b,c,e,f:double;
BEGIN
...
FUNCTION DataJD(r,m,d:double):double;
VAR a,b,c,e:double;
BEGIN
IF r<0 THEN r:=r+1;
a:=4716+r+int((m+9)/12);
...
VAR x:double; a,a1,r1,m1,d1,y:INTEGER; ok:BOOLEAN;
...
FUNCTION RefInOut(z:double):double;
...
FUNCTION RefOutIn(z:double):double;
...
PROCEDURE OblGodz(jd:double;VAR rb,mb,db,g,m,s:INTEGER);
VAR gr:double;
BEGIN
...
FUNCTION Deg(d:double):s15;
VAR g,m,s:WORD; gr:double;
BEGIN
 gr:=d/(2*Pi);
...
VAR i,j,wsp:INTEGER; p,w,godz,si,co:double; c:CHAR; ss:S9;
BEGIN
...
4,-4,-3,3,-2,-2,2,2,2,2);
CONST tab3r:ARRAY[94..123]OF double=
...
VAR i,j:INTEGER; p,w,godz,si,co:double; c:CHAR; ss:S9;
BEGIN
...
FUNCTION Faza(obiekt:BYTE):double;
BEGIN
  Faza:=
...
FUNCTION LoFaza(obiekt:BYTE):double;
BEGIN
  LoFaza:=
...
FUNCTION LoDiam(obiekt:BYTE):double; { srednica katowa }
BEGIN
  CASE obiekt OF
...
FUNCTION OblFAZE(jd:double;dir:INTEGER):double;  { Patrz Mlody Technik 12/1985, s.48 }
VAR k,t,tr,M,M1,F,ss,jd1:double; i,dk:INTEGER;
BEGIN
jd1:=jd;
i:=0;
REPEAT
...
FUNCTION SunAng(jd:double):double;
VAR pom:double;
BEGIN
...
FUNCTION MoonAng(jd:double):double;
BEGIN
  t1:=jd-2451545.0+dTT(jd);
  OblJR;
  OblSLON;
...
FUNCTION OblZacmKs(jd:double;dir:INTEGER):double;  { Patrz Mlody Technik 8/1986, s.92 }
LABEL LOOP, ENDLOOP;
VAR cc,k,k1,kp,gamma,Hd,hm,u,t,tr,M,M1,F,ss,jd1:double; i,dk:INTEGER;
BEGIN
  jd1:=jd;
  i:=0;
...
FUNCTION OblZacmSl(jd:double;dir:INTEGER):double;  { Patrz Mlody Technik 9/1986, s.80 }
LABEL LOOP, ENDLOOP;
VAR cc,k,k1,kp,gamma,Hd,hm,u,t,tr,M,M1,F,ss,jd1:double; i,dk:INTEGER;
BEGIN
  jd1:=jd;
...
FUNCTION ss(q,x:double):INTEGER;
VAR x1,x2:double;
BEGIN
...
PROCEDURE OblWSCH(tb:double);
VAR g,t11,t12:double; s,s1,s2:INTEGER; v:BOOLEAN;
BEGIN
...
BEGIN
 ...
 REPEAT
  ...
  IF UpCase(c) IN ['0','F','E','A','S','J','R','Y','N','M','+','-','D','H','Z','1'..'7','W',#27,#1] THEN
  BEGIN
   GoToXY(1,25); ClrEol;
   czasy[0]:=Czas;
   HideCursor;
